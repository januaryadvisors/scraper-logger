...
ROW_ID=$(curl -H "Content-Type: application/json" -X POST -d '{"start_time": "'"$START_TIME"'", "name": "'"$SCRAPER_NAME"'"}' http://localhost:3000/start)
...
curl -H "Content-Type: application/json" -X POST -d '{"end_time":"'"$END_TIME"'","status":"'"$STATUS"'","id":"'"$ROW_ID"'"}' http://localhost:3000/end
